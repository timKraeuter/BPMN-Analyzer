...
public class BPMNToGrooveTransformer implements GrooveTransformer<BPMNCollaboration> {
    ...
    static void addOutgoingMessagesForFlowNode(FlowNode producingMessageFlowNode,
    ...
    private void generateRulesForProcess(BPMNCollaboration collaboration,
        ...
        process.getControlFlowNodes().forEach(node -> node.accept(new FlowNodeVisitor() {
            ...
            public void handle(Task task) {
                createTaskRules(process, ruleBuilder, task);
            }

            @Override
            public void handle(SendTask sendTask) {
                createTaskRules(process,
                                ruleBuilder,
                                sendTask,
            ...
            public void handle(ReceiveTask receiveTask) {
                if (receiveTask.isInstantiate()) {
                    ...
                    createInstantiateReceiveTaskRule(process, ruleBuilder, collaboration, receiveTask);
                    return;
                }
                ...
                createEndTaskRule(process, ruleBuilder, receiveTask, (noop) -> {
                });
            }
            ...
            public void handle(CallActivity callActivity) {
                ...
                createTerminateSubProcessRule(process, ruleBuilder, callActivity);

                // Generate rules for the sub process
                createRulesForExecutingTheSubProcess(ruleBuilder, callActivity, collaboration, visitedProcessModels);

                createBoundaryEventRules(process, ruleBuilder, callActivity);
            }

            @Override
            public void handle(ExclusiveGateway exclusiveGateway) {
                createExclusiveGatewayRules(process, ruleBuilder, exclusiveGateway);
            }

            @Override
            public void handle(ParallelGateway parallelGateway) {
                createParallelGatewayRule(process, ruleBuilder, parallelGateway);
            }

            @Override
            public void handle(InclusiveGateway inclusiveGateway) {
                createInclusiveGatewayRules(process, ruleBuilder, inclusiveGateway);
            }

            @Override
            public void handle(EndEvent endEvent) {
                createEndEventRule(process, ruleBuilder, endEvent, collaboration);
            }

            @Override
            public void handle(EventBasedGateway eventBasedGateway) {
                createEventBasedGatewayRule(eventBasedGateway, ruleBuilder, process);
            }
            ...
            public void handle(IntermediateCatchEvent intermediateCatchEvent) {
                createIntermediateCatchEventRule(intermediateCatchEvent, ruleBuilder, process, collaboration);
            }
        }));
    ...
    void createInstantiateReceiveTaskRule(AbstractProcess process,
                                          GrooveRuleBuilder ruleBuilder,
                                          BPMNCollaboration collaboration,
                                          ReceiveTask receiveTask) {
        ...
        createTaskRules(process, ruleBuilder, receiveTask);
    }

    void createTaskRules(AbstractProcess process, GrooveRuleBuilder ruleBuilder, AbstractTask task) {
        createTaskRules(process, ruleBuilder, task, (noop) -> {
        });
    }
    ...
    private void createTaskRules(AbstractProcess process,
        ...
        createEndTaskRule(process, ruleBuilder, task, endTaskRuleAdditions);
    }
    ...
    private void createRulesForExecutingTheSubProcess(GrooveRuleBuilder ruleBuilder,
                                                      CallActivity callActivity,
                                                      BPMNCollaboration collaboration,
                                                      Set<Process> visitedProcessModels) {
    ...
    private void createTerminateSubProcessRule(AbstractProcess process,
                                               GrooveRuleBuilder ruleBuilder,
                                               CallActivity callActivity) {
    ...
    private void createExclusiveGatewayRules(AbstractProcess process,
                                             GrooveRuleBuilder ruleBuilder,
    ...
    private void createEventBasedGatewayRule(EventBasedGateway eventBasedGateway,
                                             GrooveRuleBuilder ruleBuilder,
                                             AbstractProcess process) {
    ...
    void createStartEventRule(StartEvent startEvent,
                              GrooveRuleBuilder ruleBuilder,
                              Process process,
                              BPMNCollaboration collaboration) {
        switch (startEvent.getType()) {
