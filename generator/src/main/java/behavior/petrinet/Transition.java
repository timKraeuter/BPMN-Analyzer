...
public class Transition {
  ...
  public Set<Place> getNextPlaces() {
    return this.outgoingEdges.stream().map(Pair::getValue).collect(Collectors.toSet());
  }

  public Set<Place> getPreviousPlaces() {
    return this.incomingEdges.stream().map(Pair::getValue).collect(Collectors.toSet());
  }
}
