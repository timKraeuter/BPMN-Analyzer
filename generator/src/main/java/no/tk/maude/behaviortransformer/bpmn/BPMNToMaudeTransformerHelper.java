...
public interface BPMNToMaudeTransformerHelper {
  ...
  default String getFlowNodeRuleNameWithIncFlow(FlowNode flowNode, String incomingFlowId) {
    if (flowNode.getIncomingFlows().count() > 1) {
      return String.format(BPMNToMaudeTransformerConstants.RULE_NAME_NAME_ID_FORMAT, getFlowNodeRuleName(flowNode), incomingFlowId);
    }
  ...
  default String getFlowNodeRuleName(FlowNode flowNode) {
    ...
    return String.format(BPMNToMaudeTransformerConstants.RULE_NAME_NAME_ID_FORMAT, flowNode.getName(), flowNode.getId());
  }

  default String getTokenForFlowNode(FlowNode flowNode) {
    return getTokenOrSignalOccurrenceForFlowNode(flowNode, BPMNToMaudeTransformerConstants.TOKEN_FORMAT_ONLY_ID, BPMNToMaudeTransformerConstants.TOKEN_FORMAT);
  }

  default String getSignalOccurrenceForFlowNode(FlowNode flowNode) {
    return getTokenOrSignalOccurrenceForFlowNode(
        flowNode, BPMNToMaudeTransformerConstants.SIGNAL_OCCURRENCE_FORMAT_ONLY_ID, BPMNToMaudeTransformerConstants.SIGNAL_OCCURRENCE_FORMAT);
  }
  ...
  default String getSignalOccurrenceForSequenceFlow(SequenceFlow sequenceFlow) {
    return getTokenOrSignalOccurrenceForSequenceFlow(sequenceFlow, BPMNToMaudeTransformerConstants.SIGNAL_OCCURRENCE_FORMAT);
  }
  ...
  default String getTokenForSequenceFlow(SequenceFlow sequenceFlow) {
    return getTokenOrSignalOccurrenceForSequenceFlow(sequenceFlow, BPMNToMaudeTransformerConstants.TOKEN_FORMAT);
  }

  default MaudeObject createTerminatedProcessSnapshot(AbstractBPMNProcess process) {
    String oid = "OT";
    getRuleBuilder().addVar(BPMNToGrooveTransformerConstants.OIDS, BPMNToGrooveTransformerConstants.OID, oid);
    // Is a subprocess. Thus, should not have parents!
    return createProcessSnapshotObjectWithoutParents(process, oid, BPMNToMaudeTransformerConstants.NONE, BPMNToMaudeTransformerConstants.NONE, BPMNToMaudeTransformerConstants.NONE, BPMNToMaudeTransformerConstants.TERMINATED);
  }
  ...
  default MaudeObject createProcessSnapshotObjectNoSubProcessAndSignals(
    ...
    return createProcessSnapshotObject(process, oid, BPMNToMaudeTransformerConstants.NONE, tokens, BPMNToMaudeTransformerConstants.NONE, BPMNToMaudeTransformerConstants.RUNNING);
  }

  default MaudeObject createProcessSnapshotObjectNoSubProcessAndSignals(
      AbstractBPMNProcess process, String tokens) {
    return createProcessSnapshotObjectRunning(process, BPMNToMaudeTransformerConstants.NONE, tokens, BPMNToMaudeTransformerConstants.NONE);
  }

  default MaudeObject createProcessSnapshotObjectAnySubProcessAndSignals(
      AbstractBPMNProcess process, String tokens) {
    return createProcessSnapshotObjectRunning(process, BPMNToMaudeTransformerConstants.ANY_SUBPROCESSES, tokens, BPMNToMaudeTransformerConstants.ANY_SIGNALS);
  }

  default MaudeObject createProcessSnapshotObjectAnySubProcessAndNoSignals(
      AbstractBPMNProcess process, String tokens) {
    return createProcessSnapshotObjectRunning(process, BPMNToMaudeTransformerConstants.ANY_SUBPROCESSES, tokens, BPMNToMaudeTransformerConstants.NONE);
  }
  ...
  default MaudeObject createProcessSnapshotObjectRunning(
    ...
    return createProcessSnapshotObject(process, subprocesses, tokens, signals, BPMNToMaudeTransformerConstants.RUNNING);
  }
  ...
  default MaudeObject createProcessSnapshotObjectWithParents(
    ...
    MaudeObject processObject =
        createProcessSnapshotObjectWithoutParents(
            process, BPMNToGrooveTransformerConstants.O + 0, subprocesses, tokens, BPMNToMaudeTransformerConstants.ANY_SIGNALS, BPMNToMaudeTransformerConstants.RUNNING);
  ...
  default MaudeObject wrapInParentIfNeeded(
    ...
    getRuleBuilder().addVar(BPMNToGrooveTransformerConstants.OIDS, BPMNToGrooveTransformerConstants.OID, oid);
    getRuleBuilder().addVar(BPMNToGrooveTransformerConstants.TOKENS, BPMNToGrooveTransformerConstants.MSET, anyTokens);
    getRuleBuilder().addVar(BPMNToGrooveTransformerConstants.SIGNALS, BPMNToGrooveTransformerConstants.MSET, anySignals);
    getRuleBuilder().addVar(BPMNToGrooveTransformerConstants.SUBPROCESSES, BPMNToGrooveTransformerConstants.CONFIGURATION, anySubprocesses);

    MaudeObject parentProcessObject =
        createProcessSnapshotObjectWithoutParents(
            parentProcess,
            oid,
            processObject.generateObjectString() + BPMNToMaudeTransformerConstants.WHITE_SPACE + anySubprocesses,
            anyTokens,
            anySignals,
  ...
  default MaudeObject createProcessSnapshotObject(
      ...
      String signals,
      String state) {
    return createProcessSnapshotObject(process, BPMNToGrooveTransformerConstants.O + 0, subprocesses, tokens, signals, state);
  }
  ...
  default MaudeObject createProcessSnapshotObject(
    ...
    MaudeObject processObject =
        getObjectBuilder()
            ...
            .addAttributeValue("name", String.format(BPMNToMaudeTransformerConstants.ENQUOTE_FORMAT, process.getName()))
            .addAttributeValue("tokens", String.format(BPMNToMaudeTransformerConstants.BRACKET_FORMAT, tokens))
            .addAttributeValue("signals", String.format(BPMNToMaudeTransformerConstants.BRACKET_FORMAT, signals))
            .addAttributeValue("subprocesses", String.format(BPMNToMaudeTransformerConstants.BRACKET_FORMAT, subprocesses))
  ...
  default String getMessageForFlow(MessageFlow messageFlow) {
    return String.format(BPMNToMaudeTransformerConstants.TOKEN_FORMAT, messageFlow.getNameOrDescriptiveName(), messageFlow.getId());
  }
  ...
  default void addSendMessageBehaviorForFlowNode(FlowNode messageSource) {
    ...
    for (MessageFlow messageFlow : getCollaboration().outgoingMessageFlows(messageSource)) {
      if (messageFlow.getTarget().isInstantiateFlowNode()
          || BPMNToGrooveTransformerHelper.isAfterInstantiateEventBasedGateway(messageFlow.getTarget())) {
  ...
  default void addMessageFlowInstantiateFlowNodeBehavior(
    ...
    String tokens = getTokenForFlowNode(mFlowTarget) + BPMNToMaudeTransformerConstants.ANY_OTHER_TOKENS;
    getRuleBuilder()
    ...
    interactionNode
        ...
        .forEach(
            incomingFlow -> {
              ...
              String preTokens = getTokenForSequenceFlow(incomingFlow) + BPMNToMaudeTransformerConstants.ANY_OTHER_TOKENS;
              getRuleBuilder()
                  .addPreObject(
                      createProcessSnapshotObjectAnySubProcessAndSignals(process, preTokens));

              String postTokens = getTokenForFlowNode(interactionNode) + BPMNToMaudeTransformerConstants.ANY_OTHER_TOKENS;
              getRuleBuilder()
                  .addPostObject(
  ...
  default void createEndInteractionNodeRule(FlowNode interactionNode, AbstractBPMNProcess process) {
    ...
    incomingMessageFlows.forEach(
        messageFlow -> {
          ...
          String preTokens = getConsumedTokenForInteractionNode(interactionNode) + BPMNToMaudeTransformerConstants.ANY_OTHER_TOKENS;
          getRuleBuilder()
              .addPreObject(
                  createProcessSnapshotObjectWithParents(process, BPMNToMaudeTransformerConstants.ANY_SUBPROCESSES, preTokens));
          addMessageConsumption(messageFlow);

          String postTokens = getOutgoingTokensForFlowNode(interactionNode) + BPMNToMaudeTransformerConstants.ANY_OTHER_TOKENS;
          getRuleBuilder()
              .addPostObject(
                  createProcessSnapshotObjectWithParents(process, BPMNToMaudeTransformerConstants.ANY_SUBPROCESSES, postTokens));
