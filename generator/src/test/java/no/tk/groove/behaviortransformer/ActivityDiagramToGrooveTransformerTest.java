...
import no.tk.behavior.activity.expression.bool.BooleanBinaryOperator;
import no.tk.behavior.activity.expression.bool.BooleanUnaryExpression;
import no.tk.behavior.activity.expression.bool.BooleanUnaryOperator;
import no.tk.behavior.activity.expression.integer.IntegerCalculationExpression;
import no.tk.behavior.activity.expression.integer.IntegerCalculationOperator;
import no.tk.behavior.activity.expression.integer.IntegerComparisonExpression;
import no.tk.behavior.activity.expression.integer.IntegerComparisonOperator;
import no.tk.behavior.activity.nodes.ActivityFinalNode;
import no.tk.behavior.activity.nodes.ActivityNode;
import no.tk.behavior.activity.nodes.DecisionNode;
import no.tk.behavior.activity.nodes.ForkNode;
import no.tk.behavior.activity.nodes.InitialNode;
import no.tk.behavior.activity.nodes.JoinNode;
import no.tk.behavior.activity.nodes.MergeNode;
import no.tk.behavior.activity.nodes.OpaqueAction;
...
class ActivityDiagramToGrooveTransformerTest extends BehaviorToGrooveTransformerTestHelper {
  ...
  /**
   * Tests an activity diagram consisting of two subsequent activities. See
  ...
  void testVariables() throws Exception {
    ...
    IntegerCalculationExpression sum =
        new IntegerCalculationExpression(x, y, sumVar, IntegerCalculationOperator.ADD);
  ...
  void testExpression() throws Exception {
    ...
    IntegerCalculationExpression diff =
        new IntegerCalculationExpression(x, y, diffVar, IntegerCalculationOperator.SUBTRACT);
    IntegerComparisonExpression x_equals_x =
        new IntegerComparisonExpression(x, x, xEqualsX, IntegerComparisonOperator.EQUALS);
    IntegerComparisonExpression x_seq_x =
        new IntegerComparisonExpression(x, x, xSeqX, IntegerComparisonOperator.SMALLER_EQUALS);
    IntegerComparisonExpression x_smaller_x =
        new IntegerComparisonExpression(x, x, xSmallerX, IntegerComparisonOperator.SMALLER);
    IntegerComparisonExpression x_geq_x =
        new IntegerComparisonExpression(x, x, xGeqX, IntegerComparisonOperator.GREATER_EQUALS);
    IntegerComparisonExpression x_greater_x =
        new IntegerComparisonExpression(x, x, xGreaterX, IntegerComparisonOperator.GREATER);
    BooleanUnaryExpression notAExp = new BooleanUnaryExpression(a, notA, BooleanUnaryOperator.NOT);
    BooleanBinaryExpression aAndBExp =
        new BooleanBinaryExpression(a, b, BooleanBinaryOperator.AND, aAndB);
    BooleanBinaryExpression aOrBxp =
        new BooleanBinaryExpression(a, b, BooleanBinaryOperator.OR, aOrB);

    OpaqueAction action1 =
        new OpaqueAction(
            "Action1",
            Lists.newArrayList(
                x_equals_x,
  ...
  void testTTCExample() throws Exception {
    ...
    BooleanUnaryExpression notInternalExp =
        new BooleanUnaryExpression(not_internal, internal, BooleanUnaryOperator.NOT);
    ...
    OpaqueAction get_welcome_package =
        new OpaqueAction("get_welcome_package", Lists.newArrayList());
    OpaqueAction assign_to_project_external =
        new OpaqueAction("assign_to_project_external", Lists.newArrayList());
    ForkNode forkNode = new ForkNode("fork");
  ...
  void perf1() throws Exception {
    ...
    builder.setName("perf1").setInitialNode(initNode);
  ...
  void perf3_2() throws Exception {
    ...
    OpaqueAction a =
        new OpaqueAction(
            "a",
            Lists.newArrayList(
                new IntegerComparisonExpression(
                    i_var, value2, iG2, IntegerComparisonOperator.GREATER),
                new IntegerComparisonExpression(
                    i_var, value2, iE2, IntegerComparisonOperator.EQUALS),
                new IntegerComparisonExpression(
                    i_var, value2, iL2, IntegerComparisonOperator.SMALLER)));
    ...
    OpaqueAction j =
        new OpaqueAction(
            ...
            Lists.newArrayList(
                new IntegerComparisonExpression(
                    loop, iterations, loopEiterations, IntegerComparisonOperator.EQUALS),
                new IntegerComparisonExpression(
                    loop, iterations, loopLiterations, IntegerComparisonOperator.SMALLER),
                new IntegerComparisonExpression(
                    loop, iterations, loopEiterations, IntegerComparisonOperator.EQUALS)));
    ...
    OpaqueAction l =
        new OpaqueAction(
            "l",
            Lists.newArrayList(
                new IntegerCalculationExpression(
                    loop, value1, loop, IntegerCalculationOperator.ADD)));
  ...
  void myPerf() throws Exception {
    ...
    OpaqueAction a =
        new OpaqueAction(
            ...
            Lists.newArrayList(
                new IntegerCalculationExpression(
                    counter, value1, counter, IntegerCalculationOperator.ADD),
                new IntegerComparisonExpression(
                    counter, iterations, stop, IntegerComparisonOperator.EQUALS),
