...
export class GrooveService {
  ...
  checkBPMNSpecificProperties(bpmnSpecificPropertiesToBeChecked: string[], xmlModel: Blob) {
    const formData = new FormData();
  ...
  checkTemporalLogic(logic: string, property: string, xmlModel: Blob): Observable<ModelCheckingResponse> {
    const formData = new FormData();
    formData.append('logic', logic);
    formData.append('property', property);
    formData.append('file', xmlModel);
    return this.httpClient.post<ModelCheckingResponse>(checkTemporalLogicPropertyURL, formData);
  }
}
